...
Mocha.interfaces['gagarin'] = module.exports = function (suite) {
  ...
  suite.on('pre-require', function (context) {
    ...
    context.meteor = function (options, onStart) {
      ...
      options.pathToApp = options.pathToApp || gagarinOptions.pathToApp;
      options.helpers = myHelpers;
      options.settings = tools.getSettings(options.settings) || gagarinSettings;
      options.verbose = options.verbose !== undefined ? options.verbose : gagarinOptions.verbose;
      options.remoteServer = options.remoteServer || gagarinOptions.remoteServer;
      options.skipBuild = options.skipBuild !== undefined ? options.skipBuild : gagarinOptions.skipBuild;
      options.startupTimeout = options.startupTimeout !== undefined ? options.startupTimeout : gagarinOptions.startupTimeout;
      options.startupTimeout2 = options.startupTimeout2; // this one is only used for internal tests

      var meteor = new Meteor(options);
    ...
    context.mongo = function (options) {
      ...
      var mongo = makeMongoDB(options, options.helpers);
